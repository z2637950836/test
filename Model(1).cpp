...
void AnasaziModel::processSocialRiskPooling(std::vector<Household*>& highDebtHouseholds)
{
	...
	if (lastY != year) { lastY = year; printed = 0; }
	for (auto* borrower : agents)
	{
		std::uint64_t ki = keyOf(borrower->getId());
		...
		if (rank == 0 && printed < 1 && storage_i < 0) {
			std::cerr << "[B] year=" << year << " borrower=" << borrower->getId().id()
				<< " storage_i=" << storage_i << " A_request0=" << A_request0
				<< std::endl;
			printed++;
		}
...
void AnasaziModel::updateHouseholdProperties()
{
	...
	int highDebtCount_t = 0;

	// =========================================================
	// Stage 1：死亡 + fission + nextYear
	// ★注意：这里不再做 checkMaize/fieldSearch（自然迁出先不要发生）
	// =========================================================
	std::vector<Household*> agents;
	context.selectAgents(repast::SharedContext<Household>::LOCAL, noOfAgents, agents);

	for (auto* household : agents)
	{
			...
			fissionGen->next(), param.fertilityProbability))
		{
			...
			fieldSearch(newAgent);   // newborn 仍然找田
			houseID++;
		}

		// --- 进入下一年结算（更新 maizeStorage / age 等） ---
		household->nextYear(param.householdNeed);
	}

	// =========================================================
	// Stage 2：先借粮 / 债务 / 声誉 / 计算 D_i / B_i，并收集高债务户
	// =========================================================
	std::vector<repast::AgentId> highDebtIds;
	{
		std::vector<Household*> high_debt_households_t;
		processSocialRiskPooling(high_debt_households_t);
	...
	// =========================================================
	// Stage 2.5：你要的新逻辑
	// 借完粮仍不够（maizeStorage < 0） -> 才 fieldSearch/relocate
	// 找不到 -> 自然迁出并删除
	// =========================================================
	{
		...
		for (auto* h : survivors)
		{
			...
			// 借完以后仍然不够
			if (h->getMaizeStorage() >= 0) continue;

			repast::AgentId hid = h->getId();

			// 迁出前位置先记录（因为 fieldSearch 可能触发 remove）
			std::vector<int> loc_before;
			householdSpace->getLocation(hid, loc_before);

			bool fieldFound = fieldSearch(h);
			...
			if (!fieldFound)
			{
				...
				MigrantRecord rec;
				rec.id = hid;
				rec.location = loc_before;
				natural_records_t.push_back(rec);

				// 删除（如果 fieldSearch 内部已经删过，这里会自动跳过）
				removeHouseholdById(hid);
			}
		}
	}
	...
	// =========================================================
	{
		...
		for (auto* h : survivors)
		{
			...
			for (auto* other : survivors) {
				if (other == h) continue;
				social_sum += tieStrength(h, other);
			}
	...
	// =========================================================
	// Stage 4：Feature 3 —— 跟随迁移（natural_migrants_t 是“借完仍不够且找不到田”的人）
	// =========================================================
	...
	// 每年都 push（即使为空），保证 windowL 是连续年份窗口
